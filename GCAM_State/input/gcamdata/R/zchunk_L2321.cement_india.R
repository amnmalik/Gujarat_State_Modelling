#' module_gcamindia_L2321.cement_India
#'
#' Make the logit and input tables for the cement sector in gcam-india
#'
#' @param command API command to execute
#' @param ... other optional parameters, depending on command
#' @return Depends on \code{command}: either a vector of required inputs,
#' a vector of output names, or (if \code{command} is "MAKE") all
#' the generated outputs: \code{L2321.DeleteSupplysector_Indiacement}, \code{L2321.DeleteFinalDemand_Indiacement}, \code{L2321.StubTechProd_cement_India},
#' \code{L2321.StubTechCoef_cement_India}, \code{L2321.StubTechCalInput_cement_heat_India}, \code{L2321.StubTechMarket_cement_India}, \code{L2321.BaseService_cement_India},
#' \code{L2321.Supplysector_cement_India}, \code{L2321.FinalEnergyKeyword_cement_India}, \code{L2321.SubsectorLogit_cement_India},
#' \code{L2321.SubsectorShrwtFllt_cement_India}, \code{L2321.StubTech_cement_India}, \code{L2321.PerCapitaBased_cement_India},
#'  \code{L2321.PriceElasticity_cement_India}, \code{L2321.IncomeElasticity_cement_gcam3_India} .
#'  The corresponding file in the original data system was \code{L2321.cement_India.R} (gcam-india level2).
#' @details Make the logit and input tables for the cement sector in gcam-usa
#' @importFrom assertthat assert_that
#' @importFrom dplyr filter mutate select
#' @importFrom tidyr gather spread
#' @author AM Dec20
module_gcamindia_L2321.cement_India <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    return(c("L2321.DeleteSupplysector_Indiacement",
             ...
             "L2321.StubTechCalInput_cement_elec"))
    ...
    L1321.in_EJ_india_state_cement_F_Y <- get_data(all_data, "L1321.in_EJ_india_state_cement_F_Y")
    L1321.IO_GJkg_india_state_cement_F_Yh <- get_data(all_data, "L1321.IO_GJkg_india_state_cement_F_Yh")
    ...
    # In order to create state cement sectors on the state level for gcam-India the
    # cement sector will need to be removed from the India region in the supply sector
    ...
    # The cement_India_processing function replaces the "identical processing for loop"
    ...
    cement_India_processing <- function(data) {
        data %>%
          filter(region == gcam.india_REGION) %>%
          write_to_all_states(names = names(data), gcamindia.STATES) ->
          new_data
      return(new_data)
      }


    # Use the cement_India_processing function to check and or process the following data frames so that
    # all of the output data frames contain information for all cement producing states.
    L2321.Supplysector_cement_India <- cement_India_processing(L2321.Supplysector_cement)
    L2321.FinalEnergyKeyword_cement_India <- cement_India_processing(L2321.FinalEnergyKeyword_cement)
    L2321.SubsectorLogit_cement_India <- cement_India_processing(L2321.SubsectorLogit_cement)
    L2321.SubsectorShrwtFllt_cement_India <- cement_India_processing(L2321.SubsectorShrwtFllt_cement)
    L2321.SubsectorInterp_cement_India <- cement_India_processing(L2321.SubsectorInterp_cement)
    L2321.StubTech_cement_India <- cement_India_processing(L2321.StubTech_cement)
    L2321.PerCapitaBased_cement_India <- cement_India_processing(L2321.PerCapitaBased_cement)
    L2321.PriceElasticity_cement_India <- cement_India_processing(L2321.PriceElasticity_cement)
    L2321.IncomeElasticity_cement_gcam3_India <- cement_India_processing(L2321.IncomeElasticity_cement_gcam3)
    ...
    L1321.IO_GJkg_india_state_cement_F_Yh %>%
      ...
      L2321.IO_GJkg_india_state_cement_F_Yh
    ...
    IO_and_globaltech %>%
      ...
      select(-value) ->
      L2321.IO_GJkg_india_state_cement_F_Yh_complete
    ...
    L2321.StubTech_cement_India  %>%
      filter(supplysector %in% L2321.IO_GJkg_india_state_cement_F_Yh_complete$supplysector) %>%
    ...
    L2321.StubTechCoef_cement_India %>%
      left_join(L2321.IO_GJkg_india_state_cement_F_Yh_complete %>%
    ...
    # comes from the India level.
    india_states_subregions_new <- india_states_subregions %>%
      mutate (region = state)

    L2321.StubTechCoef_cement_India %>%
      mutate(market.name = region,
             market.name = if_else(grepl("elec", minicam.energy.input), region, market.name)) %>%
      # replace market name with the grid region name if the minicam.energy.input is
      # considered a regional fuel market
      left_join(india_states_subregions_new %>%
    ...
    L2321.StubTechCoef_cement_India %>%
      mutate(replace = if_else(minicam.energy.input %in% gcamindia.ELECT_TD_SECTORS, 1, 0),
    ...
    # filtering for supply sectors NOT included in the L2321.StubTechCoef_cement_India data frame which
    # contains electricity inputs.

    L2321.StubTechCalInput_cement_elec <- L2321.StubTechCalInput_cement_heat_India

    L2321.StubTechCalInput_cement_heat_India %>%
      filter(!supplysector %in% L2321.StubTechCoef_cement_India$supplysector) ->
      L2321.StubTechCalInput_cement_heat_India_NOelectricity
    ...
    # Fuels are from the India markets, except for regional fuel markets
    L2321.StubTechMarket_cement_India <- L2321.StubTechMarket_cement_India  %>%
      ...
      # replace market name with the grid region name if the minicam.energy.input is
      # considered a regional fuel market
      left_join_error_no_match(india_states_subregions_new %>%
                                 select(region = state, grid_region),
                               by = "region") %>%
    ...
    L2321.DeleteSupplysector_Indiacement %>%
      ...
      add_legacy_name("L2321.DeleteSupplysector_Indiacement") %>%
    ...
    L2321.DeleteFinalDemand_Indiacement %>%
      ...
      add_legacy_name("L2321.DeleteFinalDemand_Indiacement") %>%
    ...
    L2321.StubTechProd_cement_India %>%
      ...
      add_legacy_name("L2321.StubTechProd_cement_India") %>%
      add_precursors("energy/calibrated_techs", "L1321.out_Mt_india_state_cement_Yh") ->
      L2321.StubTechProd_cement_India
    ...
    L2321.StubTechCoef_cement_India %>%
      ...
      add_legacy_name("L2321.StubTechCoef_cement_India") %>%
      add_precursors("L2321.StubTech_cement", "L1321.IO_GJkg_india_state_cement_F_Yh",
                     "gcam-india/india_states_subregions", "energy/A321.globaltech_coef") ->
      L2321.StubTechCoef_cement_India
    ...
    L2321.StubTechCalInput_cement_heat_India %>%
      ...
      add_legacy_name("L2321.StubTechCalInput_cement_heat_India") %>%
      add_precursors("L2321.StubTech_cement", "L1321.IO_GJkg_india_state_cement_F_Yh",
                     "gcam-india/india_states_subregions", "L1321.in_EJ_india_state_cement_F_Y", "energy/calibrated_techs") ->
      L2321.StubTechCalInput_cement_heat_India
    ...
    L2321.StubTechMarket_cement_India %>%
      ...
      add_legacy_name("L2321.StubTechMarket_cement_India") %>%
      add_precursors("L2321.StubTech_cement", "L1321.IO_GJkg_india_state_cement_F_Yh",
                     "gcam-india/india_states_subregions", "energy/calibrated_techs") ->
      L2321.StubTechMarket_cement_India
    ...
    L2321.BaseService_cement_India %>%
      ...
      add_legacy_name("L2321.BaseService_cement_India") %>%
      add_precursors("energy/calibrated_techs", "L1321.out_Mt_india_state_cement_Yh", "energy/A321.demand") ->
      L2321.BaseService_cement_India

    L2321.Supplysector_cement_India %>%
      add_title("Supply sector information for cement sector in cement producing states") %>%
      add_units("NA") %>%
      add_comments("Expanded supply sector information to cement producing states in region India") %>%
      add_legacy_name("L2321.Supplysector_cement_India") %>%
      add_precursors("L2321.Supplysector_cement", "L1321.out_Mt_india_state_cement_Yh") ->
      L2321.Supplysector_cement_India

    L2321.FinalEnergyKeyword_cement_India %>%
      add_title("Subsector logit exponents of cement sector in cement producing states") %>%
      add_units("Unitless") %>%
      add_comments("Expanded supply sector keywords information cement producing states in region India") %>%
      add_legacy_name("L2321.FinalEnergyKeyword_cement_India") %>%
      add_precursors("L2321.FinalEnergyKeyword_cement", "L1321.out_Mt_india_state_cement_Yh") ->
      L2321.FinalEnergyKeyword_cement_India

    L2321.SubsectorLogit_cement_India %>%
      add_title("Supply sector keywords for cement sector in cement producing states") %>%
      add_units("Unitless") %>%
      add_comments("Expanded subsector logit exponents to to cement producing states in region India") %>%
      add_legacy_name("L2321.SubsectorLogit_cement_India") %>%
      add_precursors("L2321.SubsectorLogit_cement", "L1321.out_Mt_india_state_cement_Yh") ->
      L2321.SubsectorLogit_cement_India

    L2321.SubsectorShrwtFllt_cement_India %>%
      add_title("Subsector shareweights of cement sector in cement producing states") %>%
      add_units("Unitless") %>%
      add_comments("Expanded subsector shareweights to to cement producing states in region India") %>%
      add_legacy_name("L2321.SubsectorShrwtFllt_cement_India") %>%
      add_precursors("L2321.SubsectorShrwtFllt_cement", "L1321.out_Mt_india_state_cement_Yh") ->
      L2321.SubsectorShrwtFllt_cement_India

    L2321.SubsectorInterp_cement_India %>%
      add_title("Subsector shareweight interpolation of cement sector in cement producing states") %>%
      add_units("NA") %>%
      add_comments("Expanded subsector shareweight interpolation to cement producing states in region India") %>%
      add_legacy_name("L2321.SubsectorInterp_cement_India") %>%
      add_precursors("L2321.SubsectorInterp_cement", "L1321.out_Mt_india_state_cement_Yh") ->
      L2321.SubsectorInterp_cement_India

    L2321.StubTech_cement_India %>%
      add_title("Identification of stub technologies of cement in cement producing states") %>%
      add_units("NA") %>%
      add_comments("Expanded identification of stub technologies of cement to cement producing states in region India") %>%
      add_legacy_name("L2321.StubTech_cement_India") %>%
      add_precursors("L2321.StubTech_cement", "L1321.out_Mt_india_state_cement_Yh") ->
      L2321.StubTech_cement_India

    L2321.PerCapitaBased_cement_India %>%
      add_title("Per-capita based flag for cement exports final demand in cement producing states") %>%
      add_units("NA") %>%
      add_comments("Expanded Per-capita based flag for cement exports final demand to cement producing states in region India") %>%
      add_legacy_name("L2321.PerCapitaBased_cement_India") %>%
      add_precursors("L2321.PerCapitaBased_cement", "L1321.out_Mt_india_state_cement_Yh") ->
      L2321.PerCapitaBased_cement_India

    L2321.PriceElasticity_cement_India %>%
      add_title("Price elasticity for cement in cement producing states") %>%
      add_units("Unitless") %>%
      add_comments("Expanded price elasticity for cement to cement producing states in region India") %>%
      add_legacy_name("L2321.PriceElasticity_cement_India") %>%
      add_precursors("L2321.PriceElasticity_cement", "L1321.out_Mt_india_state_cement_Yh") ->
      L2321.PriceElasticity_cement_India
    ...
    L2321.IncomeElasticity_cement_gcam3_India %>%
      ...
      add_units("Unitless") %>%
      add_comments("Expanded price elasticity for cement to cement producing states in region India") %>%
      add_legacy_name("L2321.IncomeElasticity_cement_gcam3_India") %>%
      add_precursors("L2321.IncomeElasticity_cement_gcam3", "L1321.out_Mt_india_state_cement_Yh") ->
      L2321.IncomeElasticity_cement_gcam3_India

    L2321.StubTechCalInput_cement_elec %>%
      add_title("Stub tech with elec") %>%
      add_units("EJ") %>%
      add_comments("Created to account for elec in GRIDR") %>%
      add_legacy_name("L2321.StubTechCalInput_cement_elec") %>%
      add_precursors("L2321.StubTech_cement", "L1321.IO_GJkg_india_state_cement_F_Yh",
                     "gcam-india/india_states_subregions", "L1321.in_EJ_india_state_cement_F_Y", "energy/calibrated_techs") ->
      L2321.StubTechCalInput_cement_elec

    return_data(L2321.DeleteSupplysector_Indiacement, L2321.DeleteFinalDemand_Indiacement,
                L2321.StubTechProd_cement_India, L2321.StubTechCoef_cement_India,
                L2321.StubTechCalInput_cement_heat_India, L2321.StubTechMarket_cement_India,
                L2321.BaseService_cement_India, L2321.Supplysector_cement_India,
                L2321.FinalEnergyKeyword_cement_India, L2321.SubsectorLogit_cement_India,
                L2321.SubsectorShrwtFllt_cement_India, L2321.SubsectorInterp_cement_India,
                L2321.StubTech_cement_India, L2321.PerCapitaBased_cement_India,
                L2321.PriceElasticity_cement_India, L2321.IncomeElasticity_cement_gcam3_India,
                L2321.StubTechCalInput_cement_elec)
  } else {
