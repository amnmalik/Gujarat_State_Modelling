...
module_gcamindia_L226.en_distribution <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    return(c(FILE = "gcam-india/india_states_subregions",
             ...
             FILE = "gcam-india/A26.india_state_supplysector_elect_td",
             FILE = "gcam-india/A26.india_state_subsector_logit_elect_td",
             FILE = "gcam-india/A26.india_state_techcost_elect_td",
             FILE = "gcam-india/A26.india_state_SubsectorInterp_elect_td",
             "L202.CarbonCoef",
    ...
    A26.india_state_supplysector_elect_td <- get_data(all_data, "gcam-india/A26.india_state_supplysector_elect_td")
    A26.india_state_subsector_logit_elect_td <- get_data(all_data, "gcam-india/A26.india_state_subsector_logit_elect_td")
    A26.india_state_techcost_elect_td <- get_data(all_data, "gcam-india/A26.india_state_techcost_elect_td")
    A26.india_state_StubTechCoef_electd <- get_data(all_data, "gcam-india/A26.india_state_StubTechCoef_electd")
    A26.india_state_SubsectorInterp_elect_td <- get_data(all_data, "gcam-india/A26.india_state_SubsectorInterp_elect_td")
    ...
    #Prices in this section are not exact prices from the data, so need to update them in A26 file, for now we are taking JHarkhand as a replacement of Hawaii
    ...
    A26.india_state_energy_prices %>%
      ...
      mutate(coal_adj = (Coal -  india_C_NG_DFO_prices$Coal) * CONV_BTU_KJ * gdp_deflator(1975, 2009),
             gas_adj =  if_else(State == "JH", gcamindia.GAS_ADJ_THRESH,
    ...
    L226.india_state_DeleteSupplysector_elec <- tibble(region = gcam.india_REGION, supplysector = gcamusa.ELECT_TD_SECTORS)
    ...
    L226.india_state_Supplysector_electd <- A26.india_state_supplysector_elect_td %>%
      write_to_all_india_states(c("region", names(A26.india_state_supplysector_elect_td)))

    L226.india_state_SubsectorLogit_electd <- A26.india_state_subsector_logit_elect_td %>%
      write_to_all_india_states(c("region", names(A26.india_state_subsector_logit_elect_td)))


    L226.india_state_SubsectorShrwtFllt_electd <- L226.india_state_SubsectorLogit_electd %>%
      mutate (share.weight = 1) %>%
      rename (year.fillout = logit.year.fillout) %>%
      select(region, supplysector, subsector, year.fillout, share.weight)

    L226.india_state_SubsectorInterp_electd <- A26.india_state_SubsectorInterp_elect_td %>%
      mutate(from.year = as.integer(from.year),
             to.year = as.integer(to.year)) %>%
      write_to_all_india_states(c("region", names(A26.india_state_SubsectorInterp_elect_td))) %>%
    ...
    if(gcamindia.USE_REGIONAL_ELEC_MARKETS) {
      ...
      L226.india_state_TechShrwt_electd <- L226.india_state_SubsectorShrwtFllt_electd %>%
        mutate(technology = subsector) %>% select(-year.fillout) %>%
        repeat_add_columns(tibble(year = MODEL_YEARS)) %>%
      ...
      L226.india_state_TechCost_electd <- A26.india_state_techcost_elect_td %>%
        write_to_all_india_states(c("region", names(A26.india_state_techcost_elect_td)))

      # L226.india_state_TechCoef_electd: Tech coefficients for electricity T&D
      L226.india_state_TechCoef_electd <- A26.india_state_StubTechCoef_electd %>%
        global_energy_to_india_electd() %>%
    ...
    if(exists("L226.india_state_TechShrwt_electd")) {
      L226.india_state_TechShrwt_electd %>%
        ...
        add_precursors("L226.india_state_SubsectorShrwtFllt_electd") ->
        L226.india_state_TechShrwt_electd
    } else {
    ...
    if(exists("L226.india_state_TechCost_electd")) {
      L226.india_state_TechCost_electd %>%
        ...
        add_precursors("gcam-india/A26.india_state_techcost_elect_td") ->
        L226.india_state_TechCost_electd
    } else {
      # If gcamindia.USE_REGIONAL_ELEC_MARKETS is FALSE,
      # indicating to resolve electricity demands at the national level,
    ...
    L226.india_state_Supplysector_electd %>%
      ...
      add_precursors("gcam-india/A26.india_state_supplysector_elect_td") ->
      L226.india_state_Supplysector_electd
    ...
    L226.india_state_SubsectorLogit_electd %>%
      ...
      add_precursors("gcam-india/A26.india_state_subsector_logit_elect_td") ->
      L226.india_state_SubsectorLogit_electd
    ...
    L226.india_state_SubsectorShrwtFllt_electd %>%
      ...
      add_precursors("L226.india_state_SubsectorLogit_electd") ->
      L226.india_state_SubsectorShrwtFllt_electd
    ...
    L226.india_state_SubsectorInterp_electd %>%
      ...
      add_precursors("gcam-india/A26.india_state_SubsectorInterp_elect_td") ->
      L226.india_state_SubsectorInterp_electd
