...
#' @details This chunk sets up the india energy electricity technology databases as well as writing out assumptions to all states/sectors/markets for shareweights and logits.
#' Calibrated outputs and I:O coefficients are updated from global values produced by \code{\link{module_gcamindia_L223.electricity}}.
...
module_gcamindia_L223.electricity <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    return(c(FILE = "gcam-india/india_states_subregions",
             ...
             FILE = "gcam-india/A23.india_state_elect_td_res_com_supplysector",
             FILE = "gcam-india/A23.india_state_elect_td_res_com_ElecReserve",
             FILE = "gcam-india/A23.india_state_elect_td_res_com_SubsectorLogit",
             FILE = "gcam-india/A23.india_state_elect_td_res_com_SubsectorShrwtFllt",
             FILE = "gcam-india/A23.india_state_elect_td_res_com_SubsectorInterpTo",
             FILE = "gcam-india/A23.india_state_elect_td_res_com_StubTech",
             FILE = "gcam-india/A23.india_state_elect_td_res_com_StubTechCapFactor",
             FILE = "gcam-india/A23.india.hydro.exoinput",
             FILE = "gcam-india/A23.india_state_tech_eff",
    ...
    return(c("L223.DeleteSubsector_indiaelec",
             ...
             "L223.india_state_StubTechFixOut_geothermal",
    ...
    A23.globaltech_eff <- get_data(all_data, "energy/A23.globaltech_eff") %>%
      filter (supplysector != "elect_td_bld")
    A23.india_state_elect_td_res_com_supplysector <- get_data(all_data, "gcam-india/A23.india_state_elect_td_res_com_supplysector")
    A23.india_state_elect_td_res_com_ElecReserve <- get_data(all_data, "gcam-india/A23.india_state_elect_td_res_com_ElecReserve")
    A23.india_state_elect_td_res_com_SubsectorLogit <- get_data(all_data, "gcam-india/A23.india_state_elect_td_res_com_SubsectorLogit")
    A23.india_state_elect_td_res_com_SubsectorShrwtFllt <- get_data(all_data, "gcam-india/A23.india_state_elect_td_res_com_SubsectorShrwtFllt")
    A23.india_state_elect_td_res_com_SubsectorInterpTo <- get_data(all_data, "gcam-india/A23.india_state_elect_td_res_com_SubsectorInterpTo")
    A23.india_state_elect_td_res_com_StubTech <- get_data(all_data, "gcam-india/A23.india_state_elect_td_res_com_StubTech")
    A23.india_state_elect_td_res_com_StubTechCapFactor <- get_data(all_data, "gcam-india/A23.india_state_elect_td_res_com_StubTechCapFactor")
    ...
    A23.india_state_tech_eff <- get_data(all_data, "gcam-india/A23.india_state_tech_eff") %>% gather_years() %>% rename(efficiency = value)
    ...
    A23.india.hydro.exoinput <- get_data(all_data, "gcam-india/A23.india.hydro.exoinput")
    ...
    L223.india_state_Supplysector_elec <- L223.Supplysector_elec %>% filter (region == "India") %>%
      filter(supplysector != "elect_td_bld") %>% bind_rows(A23.india_state_elect_td_res_com_supplysector) %>%
      write_to_all_india_states(names = c(names(.), 'region'))

    L223.india_state_ElecReserve <- L223.ElecReserve %>% filter (region == "India") %>%
      filter (supplysector != "elect_td_bld") %>% bind_rows(A23.india_state_elect_td_res_com_ElecReserve) %>%
      write_to_all_india_states(names = c(names(.), 'region'))

    L223.india_state_SubsectorLogit_elec <- L223.SubsectorLogit_elec %>% filter (region == "India") %>%
      filter (supplysector != "elect_td_bld") %>% bind_rows(A23.india_state_elect_td_res_com_SubsectorLogit) %>%
      write_to_all_india_states(names = c(names(.), 'region'))

    L223.india_state_SubsectorShrwtFllt_elec <- L223.SubsectorShrwtFllt_elec %>% filter (region == "India") %>%
      filter (supplysector != "elect_td_bld") %>% bind_rows(A23.india_state_elect_td_res_com_SubsectorShrwtFllt) %>%
      write_to_all_india_states(names = c(names(.), 'region'))

    L223.india_state_SubsectorInterpTo_elec <- L223.SubsectorInterpTo_elec %>% filter (region == "India") %>%
      filter (supplysector != "elect_td_bld")%>% bind_rows(A23.india_state_elect_td_res_com_SubsectorInterpTo) %>%
      write_to_all_india_states(names = c(names(.), 'region')) %>%
      filter (subsector != "geothermal")

    L223.india_state_StubTech_elec <- L223.StubTech_elec %>% filter (region == "India") %>%
      filter (supplysector != "elect_td_bld") %>% bind_rows(A23.india_state_elect_td_res_com_StubTech) %>%
      write_to_all_india_states(names = c(names(.), 'region'))

    L223.india_state_StubTechCapFactor_elec <- L223.StubTechCapFactor_elec %>% filter (region == "India") %>%
      filter (supplysector != "elect_td_bld") %>% bind_rows(A23.india_state_elect_td_res_com_StubTechCapFactor) %>%
      write_to_all_india_states(names = c(names(.), 'region'))


    process_india_to_states(L223.SubsectorShrwt_nuc) -> L223.india_state_SubsectorShrwt_nuc
    L223.india_state_SubsectorShrwt_renew <- process_india_to_states(L223.SubsectorShrwt_renew) %>%
      filter (subsector != "geothermal")
    L223.india_state_SubsectorInterp_elec <- process_india_to_states(L223.SubsectorInterp_elec) %>%
      filter (subsector != "geothermal")

      L223.india_state_StubTechEff_elec <- A23.india_state_tech_eff %>% mutate (region = region)
    ...
    L1231.india_state_out_EJ_elec_F_tech %>%
      ...
      mutate(share.weight.mult = pref,
             ...
             share.weight.mult = replace(share.weight.mult, state == "NL", 0),
             pref = replace(pref, state == "NL", 0)) ->
      L223.state_nuc_pref
    ...
    # calibration
    L223.in_EJ_state_elec_F_tech <- L1231.india_state_in_EJ_elec_F_tech %>%
      filter(year %in% MODEL_BASE_YEARS) %>%
    ...
    calibrated_techs_geo <- calibrated_techs %>%
      filter (subsector == "geothermal") %>% mutate (calibration = 'fixed output')

    calibrated_techs <- calibrated_techs %>% filter (subsector != "geothermal") %>%
      bind_rows (calibrated_techs_geo)

    # NOTE: Fixed output is assumed to apply in all historical years, regardless of final calibration year
    L223.out_EJ_state_elec_F_tech <- L1231.india_state_out_EJ_elec_F_tech %>%
    ...
    L223.india_state_StubTechFixOut_elec %>%
      ...
      L223.india_state_StubTechFixOut_hydro_1

    L223.india_state_StubTechFixOut_hydro <- L223.india_state_StubTechFixOut_hydro_1 %>%
      bind_cols(A23.india.hydro.exoinput, by = c(region, year)) %>%
      select(-year1, region1,fixedOutput) %>%
      mutate(fixedOutput = fixedOutput1) %>%
      select(-fixedOutput1) %>%
      select(region, supplysector, subsector, stub.technology, fixedOutput, share.weight.year, subs.share.weight, tech.share.weight, year)

    L223.india_state_StubTechFixOut_elec %>%
      filter(grepl("geothermal", stub.technology), year == max(HISTORICAL_YEARS)) %>%
      select(-year) %>%
      repeat_add_columns(tibble(year = MODEL_FUTURE_YEARS)) ->
      L223.india_state_StubTechFixOut_geothermal

    # L223.india_state_StubTechProd_elec: calibrated output of electricity generation technologies
    L223.india_state_StubTechProd_elec <- L223.calout_EJ_state_elec_F_tech %>%
      select(LEVEL2_DATA_NAMES[["StubTechYr"]], calOutputValue) %>%
      mutate(share.weight.year = year) %>%
      set_subsector_shrwt %>%
    ...
    A23.india_state_elect_td_res_com_StubTech <-  A23.india_state_elect_td_res_com_StubTech %>%
      mutate(minicam.energy.input = "distributed_solar")

    A23.globaltech_eff <- A23.globaltech_eff %>% filter (subsector != "geothermal") %>%
      select(supplysector, subsector,technology, minicam.energy.input) %>% rename (stub.technology = technology) %>%
      bind_rows(A23.india_state_elect_td_res_com_StubTech) %>% select(-region)


    L223.india_state_StubTechMarket_elec <- L223.india_state_StubTech_elec %>%
      repeat_add_columns(tibble(year = MODEL_YEARS)) %>%
      # For rooftop_pv (technology), match in distributed_solar instead of backup_electricity (minicam.energy.input)
      left_join_keep_first_only(select(A23.globaltech_eff, supplysector, subsector, stub.technology, minicam.energy.input),
                                by = c("supplysector", "subsector", "stub.technology")) %>%
      # Remove NA rows for hydro and geothermal
      na.omit %>%
    ...
    L223.india_state_StubTechMarket_elec <-  L223.india_state_StubTechMarket_elec %>%
    ...
    L223.india_state_StubTechMarket_elec_res_com_only <- L223.india_state_StubTechMarket_elec %>%
      filter (supplysector != "electricity") %>%
      select (-market.name, -minicam.energy.input) %>%
      mutate(minicam.energy.input = "backup_electricity")

    # L223.india_state_StubTechMarket_backup: market names of backup inputs to state electricity sectors
    L223.india_state_StubTechMarket_backup <- L223.GlobalIntTechBackup_elec %>%
      mutate(supplysector = sector.name, subsector = subsector.name) %>% filter (supplysector != "elect_td_bld") %>%
      write_to_all_india_states(names = c(names(.), 'region')) %>%
      select(region, supplysector = sector.name, subsector= subsector.name, stub.technology = technology, year, minicam.energy.input) %>%
      bind_rows(L223.india_state_StubTechMarket_elec_res_com_only) %>%
      mutate(market.name = gcam.india_REGION) %>%
    ...
    L223.india_state_Supplysector_elec_CEA %>%
      ...
      add_precursors("gcam-india/india_states_subregions", "gcam-india/A23.india_state_elect_td_res_com_supplysector") ->
      L223.india_state_Supplysector_elec_CEA
    ...
    L223.india_state_ElecReserve %>%
      ...
      add_precursors("L223.ElecReserve", "gcam-india/A23.india_state_elect_td_res_com_ElecReserve") ->
      L223.india_state_ElecReserve
    ...
    L223.india_state_SubsectorLogit_elec %>%
      ...
      add_precursors("L223.SubsectorLogit_elec", "gcam-india/A23.india_state_elect_td_res_com_SubsectorLogit") ->
      L223.india_state_SubsectorLogit_elec
    ...
    L223.india_state_SubsectorShrwtFllt_elec %>%
      ...
      add_precursors("L223.SubsectorShrwtFllt_elec","gcam-india/A23.india_state_elect_td_res_com_SubsectorShrwtFllt") ->
      L223.india_state_SubsectorShrwtFllt_elec
    ...
    L223.india_state_SubsectorInterpTo_elec %>%
      ...
      add_precursors("L223.SubsectorInterpTo_elec", "gcam-india/A23.india_state_elect_td_res_com_SubsectorInterpTo") ->
      L223.india_state_SubsectorInterpTo_elec
    ...
    L223.india_state_StubTech_elec %>%
      ...
      add_precursors("L223.StubTech_elec", "gcam-india/A23.india_state_elect_td_res_com_StubTech") ->
      L223.india_state_StubTech_elec
    ...
    L223.india_state_StubTechEff_elec %>%
      ...
      add_precursors("gcam-india/A23.india_state_tech_eff") ->
      L223.india_state_StubTechEff_elec
    ...
    L223.india_state_StubTechCapFactor_elec %>%
      ...
      add_precursors("L223.StubTechCapFactor_elec", "gcam-india/A23.india_state_elect_td_res_com_StubTechCapFactor") ->
      L223.india_state_StubTechCapFactor_elec
    ...
    L223.india_state_StubTechFixOut_hydro %>%
      ...
      add_precursors("L1231.india_state_out_EJ_elec_F_tech",
                     "energy/calibrated_techs",
                     "gcam-india/A23.india.hydro.exoinput") ->
      L223.india_state_StubTechFixOut_hydro

    L223.india_state_StubTechFixOut_geothermal %>%
      add_title("Fixed outputs of future geothermal electricity generation in the states") %>%
      add_units("EJ") %>%
      add_comments("This just holds it constant for now.") %>%
      add_legacy_name("L223.india_state_StubTechFixOut_geothermal") %>%
