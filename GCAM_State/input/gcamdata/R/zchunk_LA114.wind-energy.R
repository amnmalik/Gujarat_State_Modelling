...
module_energy_LA114.wind <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    #regional_min_max_price_points <- L114.RsrcCurves_EJ_ctry_wind %>%
     ...
     # mutate(max.price = generate_max_prices(base.price, mid.price, curve.exponent)) %>%
      # Group by GCAM_region_ID
    #  group_by(GCAM_region_ID) %>%
      # Calculate min and max prices for each region
    #  mutate(min.price = min(base.price), max.price = max(max.price)) %>%
