#' module_gcamindia_LB1231.elec_tech
#'
...
#' the generated outputs: \code{L1231.india_state_in_EJ_elec_F_tech}, \code{L1231.india_state_out_EJ_elec_F_tech}. The corresponding file in the
#' original data system was \code{LB1231.Elec_tech.R} (gcam-india level1).
...
module_gcamindia_LB1231.elec_tech<- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    return(c("L1231.india_state_in_EJ_elec_F_tech",
    ...
    L123.india_state_in_EJ_elec_F             <- get_data(all_data, "L123.india_state_in_EJ_elec_F")
    L123.india_state_out_EJ_elec_F            <- get_data(all_data, "L123.india_state_out_EJ_elec_F")
    ...
    L1231.india_state_in_EJ_elec_F_tech <- L1231.share_elec_F_tech %>%
      # only the fuels that use "inputs" (oil, gas, coal, biomass)
      filter(fuel %in% L1231.in_EJ_R_elec_F_tech_Yh$fuel) %>%
      left_join_error_no_match(L123.india_state_in_EJ_elec_F, by = c("state", "sector", "fuel", "year")) %>%
      # State/Technology output = technology share * state/fuel input
      mutate(value = value.x * value.y) %>%
      select(-value.x, - value.y)

    L1231.india_state_out_EJ_elec_F_tech <- L1231.share_elec_F_tech %>%
      left_join_error_no_match(L123.india_state_out_EJ_elec_F, by = c("state", "sector", "fuel", "year")) %>%
    ...
    L1231.india_state_in_EJ_elec_F_tech %>%
      ...
      add_legacy_name("L1231.india_state_in_EJ_elec_F_tech") %>%
      ...
      add_units("EJ") %>%
      add_comments("National level technology shares multiplied by state level fuel shares") %>%
      add_legacy_name("L1231.india_state_out_EJ_elec_F_tech") %>%
      add_precursors("L123.out_EJ_R_elec_F_Yh",
                     "L1231.out_EJ_R_elec_F_tech_Yh",
                     "L123.india_state_out_EJ_elec_F") ->
      L1231.india_state_out_EJ_elec_F_tech

    return_data(L1231.india_state_in_EJ_elec_F_tech, L1231.india_state_out_EJ_elec_F_tech)
  } else {
