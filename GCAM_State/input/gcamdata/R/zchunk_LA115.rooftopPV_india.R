#' module_gcamindia_LA115.rooftopPV
#'
...
#' the generated outputs: \code{L115.india_state_rsrc_rooftopPV}. The corresponding file in the
#' original data system was \code{LA115.RooftopPV.R} (gcam-india level1).
#' @details Prepare resource curves for rooftop PV (commercial and residential combined).
#' @importFrom assertthat assert_that
#' @importFrom dplyr filter mutate select
#' @importFrom tidyr gather spread
#' @author PNK Nov20
module_gcamindia_LA115.rooftopPV <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    return(c(FILE = "gcam-india/india_states_subregions",
             FILE = "gcam-india/A23.india_state_Com_PV_supply_curve",
             FILE = "gcam-india/A23.india_state_Res_PV_supply_curve"))
  } else if(command == driver.DECLARE_OUTPUTS) {
    return(c("L115.india_state_rsrc_rooftopPV"))
    ...
    A23.india_state_Com_PV_supply_curve <- get_data(all_data, "gcam-india/A23.india_state_Com_PV_supply_curve")
    A23.india_state_Res_PV_supply_curve <- get_data(all_data, "gcam-india/A23.india_state_Res_PV_supply_curve")
    ...
    filter(A23.india_state_Res_PV_supply_curve, Relative_Cost == min(Relative_Cost))$MWh /
      filter(A23.india_state_Res_PV_supply_curve, Relative_Cost == min(Relative_Cost))$MW /
      energy.HOURS_PER_YEAR -> max_resid_capacity_factor

    # Get maximum commerical capacity factor (a constant)
    filter(A23.india_state_Com_PV_supply_curve, Rel_Cost == min(Rel_Cost))$GWH * CONV_BIL_MIL /
      filter(A23.india_state_Com_PV_supply_curve, Rel_Cost == min(Rel_Cost))$MW /
    ...
    A23.india_state_Res_PV_supply_curve %>%
    ...
    A23.india_state_Com_PV_supply_curve %>%
    ...
    bind_rows(L115.pv_sc_res, L115.pv_sc_com) %>%
      left_join_error_no_match(select(india_states_subregions, state_name, state),
                               by = "state_name") %>%
      arrange(state_name) %>%
    ...
    L115.pv_sc %>%
      ...
      left_join_error_no_match(L115.india_state_rsrc_rooftopPV, by = "state") %>%
      ## output attributes...
      ...
      add_legacy_name("L115.india_state_rsrc_rooftopPV") %>%
      add_precursors("gcam-india/india_states_subregions",
                     "gcam-india/A23.india_state_Com_PV_supply_curve",
                     "gcam-india/A23.india_state_Res_PV_supply_curve") ->
      L115.india_state_rsrc_rooftopPV

    return_data(L115.india_state_rsrc_rooftopPV)
  } else {
