...
module_gcamindia_L2232.electricity_GRIDR <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    return(c(FILE = "gcam-india/india_states_subregions",
             ...
             FILE = "gcam-india/A23.india_elect_grid_coeff",
    ...
    A23.india_elect_grid_coeff     <- get_data(all_data, "gcam-india/A23.india_elect_grid_coeff")
    ...
    L2321.StubTechCalInput_cement_elec <- get_data(all_data, "L2321.StubTechCalInput_cement_elec")
   ...
   # Calculate net own use in each grid region
    ...
    electricity_fuels <- c("elect_td_ind", "elect_td_com", "elect_td_res", "elect_td_trn")

    ##Only to process cement elec consumption for elec grids
    L2321.StubTechCalInput_cement_elec_1 <- L2321.StubTechCalInput_cement_elec %>%
      mutate(region = state,
    ...
    L223.india_grid_in_EJ_sR_td_elec <- bind_rows(L232.india_state_StubTechCalInput_indenergy,
                                                  L244.india_state_StubTechCalInput_bld,
                                                  L254.StubTranTechCalInput_India,
                                                  L2321.StubTechCalInput_cement_elec_1) %>%
      filter((minicam.energy.input %in% electricity_fuels))%>%
      mutate(subsector = "electricity") %>%
      rename (state = region)


    L2232.india_grid_in_EJ_sR_td_elec <- L223.india_grid_in_EJ_sR_td_elec %>%
      filter(year %in% MODEL_BASE_YEARS) %>%
      left_join_error_no_match(select(india_states_subregions, state, grid_region), by = "state") %>%
      group_by(grid_region, year) %>%
      summarise(consumption = sum(calibrated.value)) %>%
      ungroup %>%
      left_join_error_no_match(A23.india_elect_grid_coeff, by = c("grid_region","year")) %>%
      mutate (value = consumption * coefficient) %>%
      select (grid_region, year, value) %>%
    ...
    L2232.india_grid_Production_exports_elec <- L2232.india_grid_elec_flows_CEA %>%
      mutate(calOutputValue = round(exports, 15),
             share.weight.year = year,
    ...
    L2232.india_grid_Production_imports_CEA <- L2232.india_grid_TechCoef_elec_CEA %>%
    ...
    L2232.india_grid_Production_elec_gen_CEA <- L2232.india_grid_TechCoef_elec_CEA %>%
      filter(year %in% MODEL_BASE_YEARS, market.name != gcam.india_REGION) %>%
    ...
    L2232.india_state_Supplysector_elec %>%
      ...
      add_precursors("gcam-india/A232.structure_india",
                     "gcam-india/A23.india_elect_grid_coeff") ->
      L2232.india_state_Supplysector_elec
