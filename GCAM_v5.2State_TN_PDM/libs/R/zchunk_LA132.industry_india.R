...
module_gcamindia_LA132.industry <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    gcamindia.nationalenergyindustries <- c("industrial energy use", "agriculture energy use")
    gcamindia.nationalfeedindustries <- c("industrial feedstocks")
    ...
    L132.india_state_in_EJ_ind_elecgas_adj <-  L101.india_state_EB_EJ_state_S_F %>%
      filter(sector %in% gcamindia.nationalenergyindustries, fuel %in% c("electricity", "gas")) %>%
    ...
    L101.india_state_EB_EJ_state_S_F %>%
      filter(sector %in% gcamindia.nationalenergyindustries,
    ...
    L132.india_state_in_EJ_indfeed_liq_unscaled <- L101.india_state_EB_EJ_state_S_F %>%
      filter(sector %in% gcamindia.nationalfeedindustries,
