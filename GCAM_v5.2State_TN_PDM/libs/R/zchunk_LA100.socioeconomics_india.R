...
module_gcamindia_LA100.socioeconomics <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    return(c(FILE = "gcam-india/A10.SE_SGDP_MOSPI_11INR",
    ...
    return(c("L100.pcGDP_thous90usd_state_india",
             ...
             "L100.india_state_pop_ruralurban_old",
             "L100.india_state_pcGDP_thous90usd_ruralurban_old"))
  } else if(command == driver.MAKE) {

    state <- year <- value  <- Area <- population <- iso <-
    ...
    A10.india_pop_rural_urban_share <- get_data(all_data, "gcam-india/A10.india_pop_rural_urban_share") %>%
      gather_years
    A10.india_GDP_rural_urban_share <- get_data(all_data, "gcam-india/A10.india_GDP_rural_urban_share") %>%
      gather_years
    ...
    L100.Pop_thous_state_ruralindia <- L100.Pop_thous_state_india %>%  gather_years() %>%
      ...
      left_join_keep_first_only(A10.india_pop_rural_urban_share, by = c("state", "sector", "year")) %>%
    ...
    L100.india_state_pop_ruralurban_old <- bind_rows(L100.Pop_thous_state_ruralindia,L100.Pop_thous_state_urbanindia,L100.Pop_thous_state_commindia) %>%
      rename (pop = population)


    #Rural-urban GDP division for the Model Base years (to be used later)

    L100.pcGDP_thous90usd_state_ruralindia <- L100.pcGDP_thous90usd_state_india %>% filter (year %in% MODEL_BASE_YEARS) %>%
      mutate (sector = 'resid rural') %>%
      left_join_error_no_match(A10.india_GDP_rural_urban_share, by = c("state", "sector")) %>%
      mutate (pcGDP = value.x * value.y) %>% select (state, sector, year = year.x, pcGDP)


    L100.pcGDP_thous90usd_state_urbanindia <- L100.pcGDP_thous90usd_state_india %>% filter (year %in% MODEL_BASE_YEARS) %>%
      mutate (sector = 'resid urban') %>%
      left_join_error_no_match(A10.india_GDP_rural_urban_share, by = c("state")) %>%
      mutate (pcGDP = value.x * (1- value.y)) %>% select (state, sector = sector.x, year = year.x, pcGDP)


    L100.pcGDP_thous90usd_state_commindia <- L100.pcGDP_thous90usd_state_india %>%
      mutate (sector = 'comm') %>%
      rename (pcGDP = value)

    L100.india_state_pcGDP_thous90usd_ruralurban_old <- bind_rows (L100.pcGDP_thous90usd_state_ruralindia, L100.pcGDP_thous90usd_state_urbanindia, L100.pcGDP_thous90usd_state_commindia) %>%
    ...
    L100.india_state_pop_ruralurban_old %>%
      ...
      add_units("thousand persons") %>%
      add_comments("State populations (in thousands) from end of history (2011) projected into future till 2100") %>%
      add_precursors("gcam-india/A10.india_pop_rural_urban_share", "L100.Pop_thous_state_india") %>%
      add_legacy_name("L100.Pop_thous_state_india") ->
      L100.india_state_pop_ruralurban_old
    ...
    L100.india_state_pcGDP_thous90usd_ruralurban_old %>%
      ...
      add_units("thousand persons") %>%
      add_comments("State populations (in thousands) from end of history (2011) projected into future till 2100") %>%
      add_precursors("gcam-india/A10.india_GDP_rural_urban_share", "gcam-india/A10.india_pop_rural_urban_share", "L100.pcGDP_thous90usd_state_india") %>%
      add_legacy_name("L100.Pop_thous_state_india") ->
      L100.india_state_pcGDP_thous90usd_ruralurban_old

    return_data(L100.pcGDP_thous90usd_state_india, L100.GDP_mil90usd_state_india, L100.Pop_thous_state_india,
                L100.india_state_pop_ruralurban_old,L100.india_state_pcGDP_thous90usd_ruralurban_old)
  } else {
